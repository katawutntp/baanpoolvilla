...
export default function HouseFormPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({
          ...
          rules: data.rules || '',
          note: data.note || '',
          detailRoom: data.detailRoom || '',
          bedrooms: data.bedrooms || 0,
          bathrooms: data.bathrooms || 0,
          maxGuests: data.maxGuests || 0,
          pricePerNight: data.pricePerNight || 0,
          farFromSea: data.farFromSea || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isEdit ? `/api/houses/${params.id}` : '/api/houses';
      ...
      if (res.ok) {
        toast.success(isEdit ? 'บันทึกสำเร็จ' : 'เพิ่มบ้านพักสำเร็จ');
        router.push('/admin/houses');
      } else {
        toast.error('เกิดข้อผิดพลาด');
      }
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-dark-100">
            {isEdit ? 'แก้ไขบ้านพัก' : 'เพิ่มบ้านพักใหม่'}
          </h1>
          <p className="text-gray-500 mt-1">
            {isEdit ? 'แก้ไขข้อมูลและรายละเอียดของบ้านพัก' : 'กรอกข้อมูลบ้านพักเพื่อแสดงบนเว็บไซต์'}
          </p>
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            >
              {saving ? (
                ...
                <>
                  <FiSave size={20} />
                  {isEdit ? 'บันทึกการแก้ไข' : 'เพิ่มบ้านพัก'}
                </>
              )}
            </button>
          </div>
